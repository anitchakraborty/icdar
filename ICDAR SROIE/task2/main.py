...
def load_images_to_predict():
    ...
    model.load_state_dict({k.replace('module.',''):v for k,v in torch.load(model_path,map_location={'cuda:0': 'cpu'}).items()})
    ...
    for jpg in jpg_files:
        ...
        words_list = []
        with open('boundingbox/'+jpg.split('/')[1].rsplit('.')[0]+'.txt', 'r') as boxes:
